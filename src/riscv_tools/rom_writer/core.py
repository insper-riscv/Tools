...
def write_rom(link: JtagLink, rom_mem_instance: int, mif_path: Path) -> None:
    ...
    rom_mem_instance : int
        In-System Memory Content Editor instance index of the ROM
        (quartus.rom_mem_instance in the project's config.yaml).
    mif_path : Path
        Path to the .mif whose content replaces the ROM's entire
        depth.
